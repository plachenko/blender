                                          ...
                                          bool geometry_set_is_instance)
{
  ...
  if (ctx->object->type != OB_MESH || geometry_set_is_instance) {
    ...
    if (mesh != nullptr) {
      DupliObject *dupli = make_dupli(ctx, ctx->object, parent_transform, instance_index++);
  ...
  if (ctx->object->type != OB_POINTCLOUD || geometry_set_is_instance) {
    ...
    if (pointcloud != nullptr) {
      DupliObject *dupli = make_dupli(ctx, ctx->object, parent_transform, 0);
  ...
  /* TODO: Make sure we don't collide with the ids above. */
  ...
  for (int64_t i : instance_offset_matrices.index_range()) {
    ...
    switch (reference.type()) {
      ...
      case InstanceReference::Type::Collection: {
        ...
        copy_dupli_context(&sub_ctx, ctx, ctx->object, nullptr, id);
        ...
        FOREACH_COLLECTION_VISIBLE_OBJECT_RECURSIVE_BEGIN (&collection, object, mode) {
          ...
          make_dupli(ctx, object, instance_matrix, object_id++);
          make_recursive_duplis(ctx, object, collection_matrix, object_id++);
        }
        FOREACH_COLLECTION_VISIBLE_OBJECT_RECURSIVE_END;
        break;
      }
      case InstanceReference::Type::GeometrySet: {
        /* TODO: Incorrect dupli ids. */
