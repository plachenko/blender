...
{
  if (b_ob_info.data_is_overwritten()) {
    return;
  }
                                   ...
                                   int motion_step)
{
  ...
  BL::MeshSequenceCacheModifier mesh_cache = object_mesh_cache_find(b_ob_info.real_object);
  if (mesh_cache) {
    return;
  }
  ...
  std::string ob_name = b_ob_info.real_object.name();
